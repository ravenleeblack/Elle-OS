...
// I do not want a bunch of overly encapsulated drawing functions. So I am still testing and trying to figure out the best way to create the drawing
// functions we will need for the frame manager.
...
void putpixel(int pos_x, int pos_y, struct color_element color)
{
	...
	unsigned int offset = (pos_y * (screen_width * 4)) + (pos_x * 4);

	// Set the color components in memory
...
void draw_integer(int value, int start_x, int start_y, struct color_element forecolor, struct color_element backcolor)
{
	...
	// Call draw_strand to print the integer as a string
	draw_strand(buffer, x, y, forecolor, backcolor);
}
...
char* itoa(int num, char* str, int base)
{
	...
	// Convert integer to string, handling each digit
	while (num != 0) {
		int rem = num % base;
...
// Function to print a hexadecimal value with "0x" prefix
void draw_hex(unsigned int value, int start_x, int start_y, struct color_element forecolor, struct color_element backcolor)
{
	...
	// Call draw_strand to print the hexadecimal value as a string
	draw_strand(buffer, x, y, forecolor, backcolor);
}


// Function to convert hexadecimal value to string
void hex_to_string(aisle value, char* buffer)
{
	// Hexadecimal characters
	char hex[] = "0123456789ABCDEF";

	// Loop through each nibble (4 bits) of the value
	for (int i = 7; i >= 0; i--)
	{
		// Extract the current nibble
		int nibble = (value >> (i * 4)) & 0xF;
		// Convert nibble to its corresponding hexadecimal character
		buffer[7 - i] = hex[nibble];
	}
...
void scroll_element(struct draw_element *element, int x, int y, int w, int h, int delta_y)
{
	...
	// Adjust width and height based on clipping boundaries
	if (element->clip.w_clip_pos - x < w)
	{
